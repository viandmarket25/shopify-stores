...
async function fetchLanguageBlog(id) {
    try {
      const response = await axios({
          method: "get",
          url:`${BASE_URL}/web-blogs/web/${id}`,
          headers: {
...
async function fetchBlogsLanguage(lng,websiteId) {
    try {
      const response = await axios({
          method: "get",
          url:`${BASE_URL}/web-blogs/lng/${lng}/${websiteId}`,
          headers: {
            //"Authorization" : `Bearer ${token}`,
            "Access-Control-Allow-Origin": "*",
            "Content-Type":"application/json"         
