...
let dataActionHttpPipe = async function (action, requestType, payload, files) {
  ...
  let promise = new Promise((resolve, reject) => {
    let __ip;
    var targetUrl = "http://127.0.0.1:8001/add-product";
    let formData = new FormData();
    ...
    for (let v = 0; v < Object.keys(tempPayload.productVariation).length; v++) {
      console.log(
        "payload options",
        tempPayload.productVariation[v]["options"]
      );
      for (
        let img = 0;
        img < tempPayload.productVariation[v]["options"].length;
        img++
      ) {
        ...
        formData.append(
          "" + v + "[]",
          tempPayload.productVariation[v]["options"][img]["images"]
        );
    ...
    $.ajax({
      ...
      processData: false,
      headers: {
        "Authorization": "Bearer " +JSON.parse(getCookie("siteAccessT")).token
      },
...
let mediaDifferentiator = (mediaSource, targetElement, type, fileObject) => {
  addProductDataModel.productMediaUniqueId =
    addProductDataModel.productMediaUniqueId + 10;
  let productMediaId = addProductDataModel.productMediaUniqueId;
  let resultElement;
  let tempContainer = elementFunction.divFunction(
    "temp-image-container-holder" + productMediaId,
    "temp-image-container-holder",
    null
  );
    let removeContainer = elementFunction.divFunction(
    "remove-container-holder" + productMediaId,
    "remove-container-holder",
    null
    );
     let fileNameContainer = elementFunction.divFunction(
    "file-name-container-holder" + productMediaId,
    "file-name-container-holder",
    fileObject.name
    );
  ...
  let imageContainer = elementFunction.divFunction(
    "temp-image-container" + productMediaId,
    "temp-image-container",
    null
  );
  let removeImageContainer = elementFunction.divFunction(
    "close-temp-image-container" + productMediaId,
    "close-temp-image-container",
    null
  );
  let tempImage = elementFunction.imgFunction(
    "temp-image" + productMediaId,
    "temp-image",
    mediaSource
  );
  let removeImage = elementFunction.imgFunction(
    "remove-image-button" + productMediaId,
    "remove-image-button",
    "http://127.0.0.1/shorpi/assets/icons/delete-icon-button.png"
  );
 ...
 // tempContainer.appendChild(removeImageContainer);
  ...
  tempContainer.appendChild(fileNameContainer);
  tempContainer.appendChild(removeContainer);
  removeContainer.appendChild(removeImageContainer);
  ...
  if (type == "product_media") {
    addProductDataModel.productMedia[
      "temp-image-container-holder" + productMediaId
    ] = mediaSource;
    addProductDataModel.productMediaFiles[
      "temp-image-container-holder" + productMediaId
    ] = fileObject;
  } else if (type == "product_media_more") {
    addProductDataModel.productMoreMedia[
      "temp-image-container-holder" + productMediaId
    ] = mediaSource;
    addProductDataModel.productMoreMediaFiles[
      "temp-image-container-holder" + productMediaId
    ] = fileObject;
  }
  //console.log(addProductDataModel)
  awaitRemoveProductMedia(
    "remove-image-button" + productMediaId,
    "temp-image-container-holder" + productMediaId,
    type,
    null
  ...
  if (input.files) {
    ...
    for (let i = 0; i < filesAmount; i++) {
      ...
      reader.onload = function (event) {
        let uiModified = mediaDifferentiator(
          event.target.result,
...
//});
let awaitSingleVariation = async (
  clickedId,
  targetElement,
  variationOptionsTarget,
  variationIsWithPrice,
  variationCategory
) => {
  $(clickedId).click(function () {
    productVariations.variationOptionId =
      productVariations.variationOptionId + 10;
...
let buildProductData = async (param1, param2) => {
  ...
  for (
    let i = 0;
    i < Object.keys(addProductDataModel.productMedia).length;
    i++
  ) {
    payload.productMedia.push(
      addProductDataModel.productMediaFiles[
        Object.keys(addProductDataModel.productMedia)[i]
      ]
    );
    pushFiles.productMedia.push(
      addProductDataModel.productMediaFiles[
        Object.keys(addProductDataModel.productMedia)[i]
      ]
    );
  ...
  for (
    let i = 0;
    i < Object.keys(addProductDataModel.productMoreMedia).length;
    i++
  ) {
    payload.productMoreMedia.push(
      addProductDataModel.productMoreMediaFiles[
        Object.keys(addProductDataModel.productMoreMedia)[i]
      ]
    );
    pushFiles.productMoreMedia.push(
      addProductDataModel.productMoreMediaFiles[
        Object.keys(addProductDataModel.productMoreMedia)[i]
      ]
    );
  ...
  for (let i = 0; i < Object.keys(addProductDataModel.products).length; i++) {
    if (
      addProductDataModel.products[
        Object.keys(addProductDataModel.products)[i]
      ] != ""
    ) {
      ...
      payload.productAttributes[Object.keys(addProductDataModel.products)[i]] =
        payloadValue;
    }
  }
  ...
  for (
    let i = 0;
    i < Object.keys(addProductDataModel.productVariations).length;
    i++
  ) {
    countVariations += 1;
    let variationTitle = $(
      "#" +
        addProductDataModel.productVariations[
          Object.keys(addProductDataModel.productVariations)[i]
        ]["title"]
    ).val();
    ...
    for (
      ...
      let title = $(
        "#" +
          addProductDataModel.productVariations[
            Object.keys(addProductDataModel.productVariations)[i]
          ]["Options"][innerKey]["title"]
      ).val();
      let image =
        addProductDataModel.productVariations[
          Object.keys(addProductDataModel.productVariations)[i]
        ]["Options"][innerKey]["image"];
  ...
  countVariations > 0
    ? (payload.productHasVariation = true)
    : (payload.productHasVariation = false);
...
let awaitBuildVariationPhotos = async (
  clickedId,
  targetElement,
  variationOptionsTarget,
  variationIsWithPrice,
...
let awaitRemoveVariationOption = async (
  clickedId,
  targetElement,
  variationOptionsTarget,
  variationIsWithPrice,
  ...
  productVariations.variationOptionId = Number(
    productVariations.variationOptionId + 10
  );
  cli = document.getElementById(clickedId);
  //console.log(cli.dataset)
  let variationBaseId = document.getElementById(
    cli.dataset.variation_base_containerid
  );
  ...
  $("body #" + clickedId).click(function () {
    ...
    delete addProductDataModel
      .productVariations[removeVariationBaseKey]["Options"][removeVariationOptionKey];
...
let awaitRemoveProductVariation = async (
  clickedId,
  targetElement,
  variationOptionsTarget,
  variationIsWithPrice,
...
let createVariationBase = async (
  ...
  if (variationCategory == "text") {
    ...
    let mainVariationBase = elementFunction.divFunction(
      "main-variation-base" + mainId,
      "main-variation-base",
      null
    );
    let mainVariationBaseTop = elementFunction.divFunction(
      "main-variation-base-top" + mainId,
      "main-variation-base-top",
      null
    );
    let mainVariationBaseContentHolder = elementFunction.divFunction(
      "main-variation-base-content-holder" + mainId,
      "main-variation-base-content-holder",
      null
    );
    let closeBaseVariation = elementFunction.imgFunction(
      "close-variation-base" + mainId,
      "close-variation-base",
      "http://127.0.0.1/shorpi/assets/icons/cancel-icon-button.png"
    );
    let singleVariationNameContainer = elementFunction.divFunction(
      "single-variation-name-container" + mainId,
      "single-variation-name-container",
      null
    );
    let singleVariationNameData = elementFunction.inputFunction(
      "single-variation-name-data" + mainId,
      "single-variation-name-data",
      "http://127.0.0.1/shorpi/assets/icons/cancel-icon-button.png",
      "Variant Title"
    );
    singleVariationNameData.setAttribute("type", "text");
    let singleVariationControlContainer = elementFunction.divFunction(
      "single-variation-control-container" + mainId,
      "single-variation-control-container",
      null
    );
    let singleVariationControlHolder = elementFunction.divFunction(
      "single-variation-control-holder" + mainId,
      "single-variation-control-holder",
      null
    );
    let singleVariationButtonHolder = elementFunction.divFunction(
      "single-variation-button-holder" + mainId,
      "single-variation-button-holder centered-position",
      null
    );
    singleVariationButtonHolder.setAttribute("align", "center");
    let singleOneMoreVariationOption = elementFunction.imgFunction(
      "single-one-more-variation-option" + mainId,
      "single-one-more-variation-option",
      "http://127.0.0.1/shorpi/assets/icons/add-icon-button-square-line.png"
    );
    singleOneMoreVariationOption.setAttribute(
      "data-variation-list-container",
      "main-variation-option-list-container" + mainId
    );
    let singleVariationLabel = elementFunction.divFunction(
      "single-variation-label" + mainId,
      "single-variation-label",
      "Add an Option"
    );
    singleVariationLabel.setAttribute("align", "left");
    let mainVariationOptionListContainer = elementFunction.divFunction(
      "main-variation-option-list-container" + mainId,
      "main-variation-option-list-container",
      null
    );
    mainVariationOptionListContainer.setAttribute(
      "data-variationcontainerid",
      "main-variation-base" + mainId
    );
    ...
    mainVariationBaseList.insertBefore(
      mainVariationBase,
      mainVariationBaseList.firstChild
    );
    ...
    mainVariationBaseTop.appendChild(singleVariationNameContainer);
       mainVariationBaseContentHolder.appendChild(
      mainVariationOptionListContainer
    );
    ...
    mainVariationBase.setAttribute(
      "data-variationContainer",
      "main-variation-base" + mainId
    );
    ...
    if (hasPrice == true) {
      ...
      addVariationData("variation", key, key, value);
    ...
    singleVariationNameData.setAttribute("type", "text");
    //mainVariationBase.setAttribute("data-variationId", mainId)
    let variationPhotoInputFile = elementFunction.inputFunction(
      "variation-photo-input-file" + mainId,
      "variation-photo-input-file",
      "http://127.0.0.1/shorpi/assets/icons/cancel-icon-button.png",
      null
    );
    variationPhotoInputFile.setAttribute("type", "file");
    variationPhotoInputFile.setAttribute("multiple", true);
    variationPhotoInputFile.setAttribute(
      "style",
      " position:relative; height:100%; width:100%; cursor:pointer; opacity:0; "
    );
    let singleVariationControlContainer = elementFunction.divFunction(
      "single-variation-control-container" + mainId,
      "single-variation-control-container",
    ...
    let singleVariationButtonHolder = elementFunction.divFunction(
      "single-variation-button-holder" + mainId,
      "single-variation-button-holder",
      null
    );
    singleVariationButtonHolder.setAttribute("align", "center");
    let singleOneMoreVariationOption = elementFunction.imgFunction(
      "single-one-more-variation-option" + mainId,
      "variation-add-icon",
      "/src/assets/icons/linear/box-add.png"
    );
    singleOneMoreVariationOption.setAttribute(
      "data-variation-list-container",
      "main-variation-option-list-container" + mainId
    ...
    singleVariationButtonHolder.setAttribute(
      "style",
      "width:160px; height:50px; "
    );
    singleOneMoreVariationOption.setAttribute(
      "style",
      "width:34px; height:30px; opacity:0.8; "
    );
    mainVariationBase.setAttribute("data-variationId", mainId);
    mainVariationBase.setAttribute(
      "data-variationContainer",
    ...
    addVariationData("variation", null, "main-variation-base" + mainId, {
      variationData: [],
      variationType: "image-text",
    });
    awaitBuildVariationPhotos(
      "#variation-photo-input-file" + mainId,
      "targetElement",
      "main-variation-option-list-container" + mainId,
      hasPrice,
...
let createProductVariationOption = async (
  targetElement,
  mainId,
  variationIsWithPrice,
  variationType,
  ...
  if (variationType == "text") {
    let mainVariationOptionListContainer =
      document.getElementById(targetElement);
    let mainVariationOptionContainer = elementFunction.divFunction(
      "main-variation-option-container" + mainId,
      "main-variation-option-container",
      null
    );
    mainVariationOptionContainer.setAttribute(
      "data-variation_option_id",
      "main-variation-option-container" + mainId
    );
    mainVariationOptionContainer.setAttribute(
      "data-variation_option_index",
      mainId
    );
    let variationOptionData = elementFunction.inputFunction(
      "variation-option-data" + mainId,
      "variation-option-data",
      ""
    );

       let variationOptionSku = elementFunction.inputFunction(
      "variation-option-sku" + mainId,
      "variation-option-data",
      "","Sku"
    );

       let variationOptionStock = elementFunction.numberInputFunction(
      "variation-option-stock" + mainId,
      "variation-option-price",
      "","Stock"
    );
    variationOptionData.setAttribute("placeholder", "Option Title");
    let variationPriceLabel = elementFunction.divFunction(
      "image-variation-variation-option-price-label-text" + mainId,
      "image-variation-variation-option-price-label-text",
      "NGN"
    );
    variationPriceLabel.setAttribute("style", "width:40px; height:40px; ");
    let variationOptionPrice = elementFunction.numberInputFunction(
      "variation-option-price" + mainId,
      "variation-option-price",
      "","Price"
    );
    let removeVariationOptionButtonContainer = elementFunction.divFunction(
      "remove-variation-option-button-container" + mainId,
      "remove-variation-option-button-container",
      null
    );
    let removeVariationOptionButton = elementFunction.imgFunction(
      "remove-variation-option-button" + mainId,
      "remove-variation-option-button",
      "http://127.0.0.1/shorpi/assets/icons/delete-icon-button.png"
    );
    ...
    mainVariationOptionContainer.appendChild(variationOptionStock);
    mainVariationOptionContainer.appendChild(variationOptionSku);
    ...
    mainVariationOptionContainer.appendChild(
      removeVariationOptionButtonContainer
    );
    removeVariationOptionButtonContainer.appendChild(
      removeVariationOptionButton
    );
    mainVariationOptionContainer.setAttribute(
      "data-variation_base_containerid",
      targetElement
    );
    mainVariationOptionContainer.setAttribute(
      "data-variation_option_containerid",
      "main-variation-option-container" + mainId
    );
    removeVariationOptionButton.setAttribute(
      "data-variation_base_containerid",
      targetElement
    );
    removeVariationOptionButton.setAttribute(
    ...
    let variationBase =
      mainVariationOptionListContainer.dataset.variationcontainerid;
    let key = "main-variation-option-container" + mainId;
    addVariationData("variation_option", variationBase, key, {
      title: "variation-option-data" + mainId,
      price: "variation-option-price" + mainId,
    });
    awaitRemoveVariationOption(
      "remove-variation-option-button" + mainId,
      "main-variation-option-container" + mainId,
      "main-variation-option-list-container" + mainId,
      "hasPrice",
    ...
    let mainVariationOptionListContainer =
      document.getElementById(targetElement);
    let imageVariationMainVariationOptionContainer =
      elementFunction.divFunction(
        "image-variation-main-variation-option-container" + mainId,
        "image-variation-main-variation-option-container",
        null
      );
    let imageVariationContainer = elementFunction.divFunction(
      "image-variation-container" + mainId,
      "image-variation-container",
      null
    );
    imageVariationMainVariationOptionContainer.setAttribute(
      "data-variation_option_id",
      "main-variation-option-container" + mainId
    );
    imageVariationMainVariationOptionContainer.setAttribute(
    ...
    let imageVariationVariationImageData = elementFunction.imgFunction(
      "image-variation-variation-image-data" + mainId,
      "image-variation-variation-image-data",
      imageSource.uiImage
    );
    let imageVariationOptionData = elementFunction.inputFunction(
      "image-variation-variation-option-data" + mainId,
      "image-variation-variation-option-data",
      ""
    );

    let imageVariationOptionSku = elementFunction.inputFunction(
      "image-variation-variation-option-sku" + mainId,
      "image-variation-variation-option-data",
      "","Sku"
    );
    let imageVariationOptionStock = elementFunction.numberInputFunction(
      "image-variation-variation-option-stock" + mainId,
      "image-variation-variation-option-price",
      "","Stock"
    );
    ...
    let imageVariationOptionPrice = elementFunction.numberInputFunction(
      "image-variation-variation-option-price" + mainId,
      "image-variation-variation-option-price",
      0.0
    );
    let variationPriceLabel = elementFunction.divFunction(
      "image-variation-variation-option-price-label" + mainId,
      "image-variation-variation-option-price-label",
      "NGN"
    );
    variationPriceLabel.setAttribute("style", "width:40px; height:40px; ");
    let removeVariationOptionButtonContainer = elementFunction.divFunction(
      "image-variation-remove-variation-option-button-container" + mainId,
      "image-variation-remove-variation-option-button-container",
      null
    );
    let removeVariationOptionButton = elementFunction.imgFunction(
      "remove-variation-option-button" + mainId,
    ...
    //if (variationIsWithPrice == true) {
      ...
      */
      imageVariationMainVariationOptionContainer.appendChild(
        imageVariationOptionPrice
      );
       imageVariationMainVariationOptionContainer.appendChild(
        imageVariationOptionStock
      );

       imageVariationMainVariationOptionContainer.appendChild(
        imageVariationOptionSku
      );
...
*/
    ...
    imageVariationContainer.appendChild(imageVariationVariationImageData);
    mainVariationOptionListContainer.appendChild(
      imageVariationMainVariationOptionContainer
    );
    imageVariationMainVariationOptionContainer.setAttribute(
      "data-variation_base_containerid",
      targetElement
